...
def get_probabilities_file(filename: str) -> dict:
    """ Gets probabilities from JSON file """
    with open(filename) as probabilities_json:
        return json.load(probabilities_json,
                         object_hook=lambda d: {int(k): d[k] for k in d})
...
def _backup_probability_get_regression_set(main_probabilities: dict) -> dict:
