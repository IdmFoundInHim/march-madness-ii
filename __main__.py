...
def randomnumbers():
    for x in urlopen('https://www.random.org/integers/?num=63&min=0&max=999&col=1&base=10&format=plain&rnd=new'):
        yield int(x)
...
if '--test' in sys.argv:
    ...
    print("Using numbers generated by random.org. Do not use faster than you could manually refresh a webpage, there is a quota per IP address per day, and you are otherwise subject to their guidelines at random.org/clients")
